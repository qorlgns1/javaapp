...
public class GoodMainMapVer {
	...
	public static void main(String[] args) {
		...
		//GoodDAO 인스턴스 생성
		GoodDAOMapVer dao = GoodDAOMapVer.sharedInstance();
		
		mainloop: while(true) {
			System.out.print("메뉴 입력:(1. 전체 데이터 보기 2. 코드로 데이터 조회하기 "
					+ "3.데이터 삽입  4.데이터 수정  5.데이터 삭제  6.이름이나 원산지로 조회 7.종료) ");
			String menu = sc.nextLine();
			
			//switch 구문 내에서 사용할 변수
			List<GoodMapVer> list = null;
			...
			case "4" :
				while(true) {
					...
					String temp = sc.nextLine();
					good.setPrice(Integer.parseInt(temp));
			...
			case "6" :
					...
					//데이터베이스 처리 메소드 호출
					List<GoodMapVer> ar = dao.search(word);
